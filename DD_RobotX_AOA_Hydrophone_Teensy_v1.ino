...
constexpr uint32_t DEAD_TIME_MS     = 1500;          // Dead time, to be adjusted based on pinger duration
constexpr float    SOUND_SPEED      = 1500.0f;       // m s⁻¹
constexpr float    HYDRO_SPACING    = 0.095f;        // Spacing between Hydrophone Mounts 
constexpr uint8_t  MED_WIN          = 5;             //  (rolling-median length)
...
void loop()
{
  ...
  /* --- compute AoA ------------------------------------------- */
  const int32_t dT_us = int32_t(t2) - int32_t(t1);      
